...
struct Node {
  ...
  Node() : id(kEmpty), port(kEmpty), is_recovery(false), id_backup(kEmpty) {}
  /** \brief node roles */
  ...
  std::string DebugString() const {
    ...
    ss << "role=" << (role == SERVER ? "server" : (role == WORKER ? "worker" : "scheduler"))
       ...
       << ", ip=" << hostname << ", port=" << port << ", is_recovery=" << is_recovery<< ", id_backup="<<id_backup;
...
struct Control {
  ...
  std::string DebugString() const {
    ...
    std::vector<std::string> cmds = {
      "EMPTY", "TERMINATE", "ADD_NODE", "BARRIER", "ACK", "HEARTBEAT","AUTOPULLRPY","ASK","REPLY", "ASK1","REPLY1","REPORT_ETH","REPORT_REPLY", "ROUTE_POLICY","ROUTE_RELEASE","RECV_REPLY","SPEED_REPORT","REPLY2"};
    std::stringstream ss;
    ss << "cmd=" << cmds[cmd];
    if (node.size()) {
      ss << ", node={";
  ...
  /** \brief all commands */
  enum Command { EMPTY, TERMINATE, ADD_NODE, BARRIER, ACK, HEARTBEAT,AUTOPULLRPY,ASK, REPLY, ASK1,REPLY1,REPORT_ETH,REPORT_REPLY, ROUTE_POLICY,ROUTE_RELEASE,RECV_REPLY,SPEED_REPORT,REPLY2 };
  /** \brief the command */
  Command cmd;
  /** \brief node infos */
...
struct Meta {
  ...
  Meta() : head(kEmpty), app_id(kEmpty), customer_id(kEmpty),
           ...
           version(kEmpty), iters(kEmpty) , sender_(kEmpty), dst(kEmpty),
           priority(kEmpty), last_hop(kEmpty) {}
  std::string DebugString() const {
    std::stringstream ss;
    if (sender_ == Node::kEmpty) {
      ss << "?";
    } else {
      ss << sender_;
    }
...
struct Message {
  ...
  void AddRoute(float val){
    meta.route.push_back(val);
  }
  /**
