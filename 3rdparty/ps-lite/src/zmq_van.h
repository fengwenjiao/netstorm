...
class ZMQVan : public Van {
 ...
 protected:
  ...
  int Bind(const Node& node, int max_retry) override {
    ...
    receiver_group.push_back(receiver_);
...
void Connect_Eth(Node &node_,Node &node) override {
    CHECK_NE(node.id, node.kEmpty);
    CHECK_NE(node.port, node.kEmpty);
    CHECK(node.hostname.size());
    int id = node.id;
    ...
    std::cout<<"dst ip info is "<<addr<<std::endl;
    if (zmq_connect(sender, addr.c_str()) != 0) {
       std::cout<<"\nconnect to "<<addr<<" failed\n";
    }else{
        std::cout<<"connect to node "<<id<<std::endl;
        senders_[id] = sender;
    }
  ...
  int RecvMsg(Message* msg,int rece_eth_id) override {
    msg->data.clear();
    size_t recv_bytes = 0;
    for (int i = 0; ; ++i) {
      ...
      while (true) {
          if (zmq_msg_recv(zmsg, receiver_group[rece_eth_id], 0) != -1) {
              break;
          }
        if (errno == EINTR) {
 ...
 private:
  ...
  std::vector<void*> receiver_group;
};
}  // namespace ps
