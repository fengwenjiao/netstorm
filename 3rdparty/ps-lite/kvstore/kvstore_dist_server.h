 ...
 */
        ...
        private:
            ...
            inline void ApplyUpdates(const DataHandleType type, const ps::Key key,
              ...
              if (!sync_mode_ || update_buf->request.size() == (size_t) ps::NumWorkers()) {
                ...
                LOG(INFO)<<"in server kvstore updater_  ";
                if (updater_) {
                  LOG(INFO)<<5002;
                ...
                stored.WaitToRead();
                if(enable_tree){
                  for(size_t i=0;i<update_buf->request.size();i++){
                    if(update_buf->request[i].sender==9){
                      auto req=update_buf->request[i];
                      ps::KVPairs<char> res;
                ...
                LOG(INFO)<<"need num is "<<(size_t) ps::NumWorkers()<<" now has num "<<update_buf->request.size();
...
//              LOG(INFO)<<"for key "<<key<<" has "<<update_buf_[key].request.size()<<" workers arrive";

//              if(update_buf_[key].request.size()==((size_t) ps::NumWorkers()-1)){
//                ps::KVMeta req;
//                req.app_id=req_meta.app_id;
//                req.customer_id=req_meta.customer_id;
////                req.request=req_meta.request;
//                req.push=!req_meta.push;
//                req.cmd=req_meta.cmd;
//                req.timestamp=req_meta.timestamp+1;
//                req.sender=9;
//                server->Response(req_meta, response);
...
//              }
            ...
            void DataHandleDefault(const DataHandleType type, const ps::KVMeta& req_meta,
              ...
              if (req_meta.push) {
                if(init_flag==0){
                  key_flag=key;
                  LOG(INFO)<<"key flag is "<<key_flag;
                }
                if(key_flag==key && init_flag<2){
                  init_flag++;
                  LOG(INFO)<<"1 init flag is "<<init_flag;
                  enable_tree = dmlc::GetEnv("ENABLE_TREE", 0);
                  LOG(INFO)<<"enable tree is "<<enable_tree;
                }
//                if(key==0 && init_flag<2){
//                  init_flag++;
//                  LOG(INFO)<<"1 init flag is "<<init_flag;
//                }
                ...
                NDArray recved = NDArray(recv_blob, 0);
                LOG(INFO)<<"2 init flag is "<<init_flag;
                if (stored.is_none() && init_flag==1) {
                  // initialization
...
//                  if(init_flag==2){
//                    while(updates.request.size()<(size_t)(ps::NumWorkers()-1)){
//                      std::this_thread::sleep_for(std::chrono::milliseconds(100));
//                    }
//                    updates.request.clear();
//                    init_flag++;
//                  }
                  if (sync_mode_ && updates.request.empty()/*updates.merged.is_none()*/) {
                      LOG(INFO)<<"a new iters";
                    stored = NDArray(dshape, Context(), false,
                                     has_multi_precision_copy(type) ? mshadow::kFloat32 : type.dtype);
                  ...
                  updates.request.push_back(req_meta);
                  LOG(INFO)<<"for key "<<key<<" worker "<<req_meta.sender<<" ,updates request size is "<<updates.request.size();
            ...
            std::unordered_map<ps::Key, NDArray> store_realt_;
            ...
            std::unordered_map<ps::Key, int> init_wait;
