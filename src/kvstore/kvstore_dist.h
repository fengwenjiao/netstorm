 ...
 private:
  ...
  void Push_(const std::vector<int>& keys,
    ...
    for (int i = 0; i < (int)uniq_keys.size(); ++i) {
      ...
      int key = uniq_keys[i];
      if(key==0){
        if(!init_finish && (get_rank() == 0)){
            init_finish=true;
        }else{
                ps_worker_->TopoUpdate();
        }
      }
      ...
      if (key==0 && seg_update_flag!=3){
        if(task_cache){
            seg_update_flag--;
            server_perf.assign(server_perf_temp.begin(),server_perf_temp.end());
            task_cache=false;
        }
        seg_update_flag++;
  ...
  void TaskDivisionUpdate(std::vector<float> perf){
      if(seg_update_flag!=3){
          if(seg_update_flag>=4){
              ...
              server_perf_temp.clear();
              server_perf_temp.assign(perf.begin(),perf.end());
              task_cache=true;
          }
          return;
      }
  ...
  std::vector<float> server_perf_temp;
