 ...
 private:
  ...
  void DataHandleDefault(const DataHandleType type, const ps::KVMeta& req_meta,
    ...
    if (req_meta.push) {
      ...
      NDArray recved = NDArray(recv_blob, 0);
      if (stored.is_none() &&!req_meta.num_merge) {
        // initialization
        ...
        LOG(INFO)<<"updates.request.size() is "<<updates.request.size();
        LOG(INFO)<<"key is "<<key<<" sync_mode_ is "<<sync_mode_<<"updates.request.empty() is "<<updates.request.empty();
        if (sync_mode_ && updates.request.empty()) {
          stored = NDArray(dshape, Context(), false,
                            has_multi_precision_copy(type) ? mshadow::kFloat32 : type.dtype);
        ...
        updates.request.push_back(req_meta);
        if(req_meta.version!=ps::MyRank()){
          LOG(INFO)<<"in server deliver";
          if(updates.request.size() == (size_t) ps_server_->sons[req_meta.version].size()){
            updates.merged.WaitToRead();
            CopyFromTo(updates.merged, store_[key]);
            store_[key].WaitToRead();
            ps::KVPairs<char> res;
            const int dtype = store_[key].dtype();
            size_t size = store_[key].shape().Size() * mshadow::mshadow_sizeof(dtype);
            char *data = static_cast<char *>(store_[key].data().dptr_);
            ps::SArray<char> vals(data, size, false);
            res.keys.push_back(key);
            res.vals = vals;
            res.lens.push_back(size);
            server->Push(req_meta, res);
          ...
          if(updates.request.size() == (size_t) ps_server_->sons[req_meta.version].size()){
            while(updates.request.size() < (size_t) ps::NumWorkers()){
              updates.request.push_back(req_meta);
            }
          }
          ApplyUpdates(type, key, &updates, server);
