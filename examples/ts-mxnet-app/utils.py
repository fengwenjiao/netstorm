...
def load_data(batch_size,
    ...
    if num_workers > 1:
        if split_by_class:
            ...
            sampler = SplitSampler(len(train), num_workers, data_slice_idx) # 将数据集中的样本划分为 num_parts 个部分，并返回第 part_index 部分的样本索引。
    else:
        sampler = None
    ...
    train_iter = gdata.DataLoader(train.transform_first(transformer), batch_size, sampler=sampler, num_workers=1)
    test_iter = gdata.DataLoader(test.transform_first(transformer), batch_size, num_workers=1)
